...
app.post("/create", function (req, res) {
  let newBookID = books.filter((b) => b.BookID == req.body.bookId);

  if (newBookID.length > 0) {
    res.writeHead(409, {
      "Content-Type": "application/plain",
    });
    res.end("Book ID already exist. Try with a new one!");
  } else {
    var newBook = {
...
app.delete("/delete/:bookId", function (req, res) {
  const index = books.findIndex((book, index) => {
  ...
  if (index === -1) {
    ...
    res.end("Book ID Not Found!! Try with another Book ID.");
  } else {
